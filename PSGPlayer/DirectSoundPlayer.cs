...
namespace midiplayer
{
   class DirectSoundPlayer : Player
   {
      ...
      public bool Init(Form form)
      {
         ...
         m_rawsamples = new byte[desc.BufferBytes / 20];
      ...
      override public int RenderSound(List<SimpleOscillator> SoundGenList)
      {
         ...
         int channelCount = SoundGenList.Count;
         for (int i = 0; i < m_rawsamples.Length; i++)
         {
            // mix all channels
            Int32 res = 0;
            for (int channels = 0; channels < SoundGenList.Count; channels++)
            {
               res += SoundGenList[channels].GetValue();
               SoundGenList[channels].m_time++;
            }

            m_rawsamples[i] = (byte)(128 + res / channelCount);
         }
