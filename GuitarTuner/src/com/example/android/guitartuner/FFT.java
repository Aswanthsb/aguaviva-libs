...
public class FFT {
  ...
  ****************************************************************/
  ...
  public void ifft(double[] x, double[] y)
  {
	  ...
	  for(short i=0;i<xx.length;i++)
	  {
		  x[i] /= (double)xx.length;
		  y[i] /= -(double)xx.length;
	  }	  
  ...
  public void ComputePower()
  {
	  for(short i=0;i<xx.length;i++)
	  {		  
		  //xx[i] = Math.sqrt( (xx[i]*xx[i]) + (yy[i]*yy[i]) );
		  xx[i] = ( (xx[i]*xx[i]) + (yy[i]*yy[i]) );
		  yy[i] = 0;
	  }  
  }
  ...
  public void FormatDataIn(short [] x)
  {
  ...
  public void FormatDataOut(short [] x)
  {
	  for(int i=0;i<x.length;i++)
	  {
		  double m = xx[i]/100; 
  ...
  public void fft(short [] x)
  {
	  ...
	  for(short i=0;i<x.length/2;i++)
	  {		  
		  double m = Math.sqrt( (xx[i]*xx[i]) + (yy[i]*yy[i]) );
